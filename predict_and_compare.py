...
#   - اگر پوشه‌هایی شامل 'cluster' یا 'noCluster' وجود داشته باشند،
#     یکی از آن‌ها به‌عنوان root مدل انتخاب می‌شود.
#   - در غیر این صورت، خود base_model_root همان root مدل است
# ============================================================== #
train_config_dirs = [
...
if train_config_dirs:
    ...
    # 📌 ریشه مدل‌ها + ریشه خروجی مخصوص همین آموزش
...
# 🔥 دیگر کل predict_linear / predict_nonlinear پاک نمی‌شود
# فقط اگر پوشه root خروجی مخصوص این اجرا وجود نداشته باشد، ساخته می‌شود
os.makedirs(output_root, exist_ok=True)

# اگر قرار است از مدل کلی استفاده شود، پوشه Global_training_with_height در خروجی هم ساخته شود
if use_global_model:
...
for h_tag in selected_heights:
    ...
    # مسیر X/Y مخصوص این ارتفاع (دادهٔ پیش‌بینی)
    ...
    # ---------------------------------------------------------- #
    # 📊 Load prediction data برای این ارتفاع
    # ---------------------------------------------------------- #
    ...
    if use_global_model:
        ...
        X_list = []
        for k in keys:
            x_gm = np.asarray(X_data[k], dtype=np.float32).reshape(-1, 1)  # GM
            T = x_gm.shape[0]
            h_col = np.full((T, 1), h_val, dtype=np.float32)               # ارتفاع ثابت
            x_feat = np.concatenate([x_gm, h_col], axis=1)                 # [GM , H]
    ...
    # 🔍 لیست سناریوها
    # ---------------------------------------------------------- #
    height_model_dir = os.path.join(model_root_dir, h_tag)

    if use_global_model:
        scenario_base_dir = global_multi_root
    else:
        scenario_base_dir = height_model_dir
    ...
    if not scenario_dirs:
        print(f"❌ هیچ پوشه سناریویی برای {h_tag} (در مسیر {'Global' if use_global_model else 'Height-specific'}) پیدا نشد. رد می‌شود.\n")
        continue

    print("📂 سناریوهای موجود برای", h_tag, ":")
    for i, nm in enumerate(scenario_dirs):
        print(f"  {i}. {nm}")

    run_all_scen = input(f"\nهمه سناریوها برای {h_tag} اجرا شوند؟ (y/n): ").strip().lower() == "y"
    ...
    if not run_all_scen:
        ...
        for item in scen_items:
            if item.isdigit():
                idx = int(item)
                if 0 <= idx < len(scenario_dirs):
                    selected_scen.append(scenario_dirs[idx])
                else:
                    invalid_scen.append(item)
            else:
                if item in scenario_dirs:
                    selected_scen.append(item)
                else:
                    invalid_scen.append(item)

        selected_scen = list(dict.fromkeys(selected_scen))
        ...
        selected_scen = scenario_dirs

    print("\n✅ سناریوهای انتخاب‌شده برای", h_tag, ":")
    for s in selected_scen:
        print("   -", s)
    print()

    # ---------------------------------------------------------- #
    # 📝 ساخت سطرهای اکسل + دیکشنری خطا برای این ارتفاع
    # ---------------------------------------------------------- #
    excel_rows = []
    excel_columns = [
    ...
    for scen_name in selected_scen:
        ...
        if not os.path.exists(model_path):
            print(f"❌ مدل یافت نشد: {model_path} → این سناریو رد می‌شود.")
            continue
        ...
        num_local = min(num_to_plot, len(Y_true_list), len(Y_pred_bc_list), len(gm_files))
        for i in range(num_local):
            ...
            all_norm_errors.extend(norm_err_series.tolist())
        ...
        if len(all_norm_errors) > 0:
            scenario_errors[scen_name] = np.array(all_norm_errors, dtype=np.float32)
