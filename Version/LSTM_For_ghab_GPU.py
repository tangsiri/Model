# -*- coding: utf-8 -*-
"""
File name      : LSTM_For_ghab_GPU.py
Author         : pc22
Created on     : Sat Dec 27 13:05:49 2025
Last modified  : Sat Dec 27 13:05:49 2025
------------------------------------------------------------
Purpose:
    Training and evaluation of LSTM-based sequence-to-sequence
    models for predicting structural response time histories
    from ground motion inputs, with optional learning across
    multiple structural heights.

    The script is designed to study the effect of training
    strategy (per-height vs. multi-height), loss weighting,
    and response linearity on model performance and stability.
------------------------------------------------------------
Description:
    This script trains deep LSTM neural networks to predict
    structural response time histories obtained from Time
    History Analysis (THA), using ground motion (GM) records
    as input.

    Two main training modes are supported:
      1) Per-height training:
         - A separate model is trained for each structural
           height.
         - Input features: ground motion only (X = [GM]).
      2) Multi-height joint training:
         - A single global model is trained using data from
           all selected heights simultaneously.
         - Structural height is added as an explicit input
           feature (X = [GM, H]).

    The script supports both linear and nonlinear structural
    responses, controlled by user input, and automatically
    selects the corresponding datasets and output directories.

    Variable-length time histories are handled using padding
    and masking. A custom weighted MSE loss function is used
    to emphasize peak response regions, controlled by user-
    defined ALPHA and THRESH parameters.

    Training is performed under multiple sensitivity-analysis
    scenarios (different epochs and loss parameters), with:
      - Deterministic behavior (fixed random seeds)
      - Automatic GPU memory management
      - Periodic checkpointing and resume capability
      - Fixed train/validation/test splits for reproducibility
------------------------------------------------------------
Inputs:
    - Ground motion time histories (X_data_*.npy)
      Located in:
        Output/3_GM_Fixed_train_linear/
        Output/3_GM_Fixed_train_nonlinear/

    - Structural response time histories from THA (Y_data_*.npy)
      Located in:
        Output/3_THA_Fixed_train_linear/
        Output/3_THA_Fixed_train_nonlinear/

    - User inputs at runtime:
        * Linear vs. nonlinear response mode
        * Selected structural heights
        * Training mode (per-height or multi-height)
------------------------------------------------------------
Outputs:
    - Trained LSTM models saved as:
        LSTM.keras

    - Periodic checkpoints:
        checkpoints/ckpt_epoch_XXXX.keras

    - Backup states for safe resume:
        backup/

    - Scaling objects for inputs and outputs:
        scaler_X_*.pkl
        scaler_Y_*.pkl
    ...
    Output directory structure is automatically organized
    based on:
        - Linear / nonlinear response
        - Training mode (per-height or global multi-height)
        - Training scenario parameters
------------------------------------------------------------
Changes since previous version:
    - Added support for joint multi-height training by
      introducing structural height as an explicit input
      feature.
    - Implemented fixed global data splitting for
      reproducibility across runs.
    - Added periodic checkpointing with automatic pruning.
    - Improved robustness against interrupted training
      sessions.
------------------------------------------------------------
Impact of changes:
    - Enables direct comparison between height-specific and
      height-agnostic learning strategies.
    - Improves training stability and reproducibility.
    - Reduces retraining cost after interruptions.
    - Provides clearer traceability of experiments for
      thesis documentation and future studies.
------------------------------------------------------------
Status:
    Stable (Research / Experimental use)

------------------------------------------------------------
Notes:
    - Padding value is fixed to -999.0 and handled via masking.
    - All random seeds are fixed to ensure deterministic runs.
    - The script assumes that preprocessing (GM and THA data
      generation) has been completed beforehand.
"""
...
choice = input("آموزش بر اساس پاسخ خطی باشد یا غیرخطی؟ برای خطی عدد 1 و برای غیرخطی عدد 0 را وارد کن: ").strip()
is_linear = (choice == "1")
...
if is_linear:
    ...
    # ⬇⬇ تغییر مهم: حالت غیرکلاستر → زیرپوشه noCluster_allHeights
    root_model_dir = os.path.join(base_dir, "Progress_of_LSTM_linear", "noCluster_allHeights")
else:
    ...
    # ⬇⬇ برای غیرخطی هم مشابه
    root_model_dir = os.path.join(base_dir, "Progress_of_LSTM_nonlinear", "noCluster_allHeights")

os.makedirs(root_model_dir, exist_ok=True)

print("📂 GM root dir :", gm_root_dir)
print("📂 THA root dir:", tha_root_dir)
print("📂 Root model dir (noCluster):", root_model_dir)
print()
...
SCENARIOS = [
    ...
    # {"EPOCHS": 20, "ALPHA": 1.0, "THRESH": 0.25},
    {"EPOCHS": 500, "ALPHA": 1.0, "THRESH": 0.5},
    {"EPOCHS": 20, "ALPHA": 3,   "THRESH": 0.5},
    # {"EPOCHS": 20, "ALPHA": 10, "THRESH": 0.5},
]
...
if not available_heights:
    raise ValueError(f"❌ هیچ پوشه‌ای به شکل H* در {gm_root_dir} پیدا نشد. مطمئن شو مرحله ۳ را اجرا کرده‌ای.")
...
print("   0 = خیر، برای هر ارتفاع جداگانه مثل قبل مدل مستقل آموزش داده شود")
print("-------------------------------------------\n")
...
# 📂 زیرپوشهٔ مخصوص «آموزش کلی با یادگیری ارتفاع»
...
if USE_MULTI_HEIGHT:
    ...
    if num_samples == 0:
        raise ValueError("❌ هیچ نمونه‌ای برای آموزش پیدا نشد (بعد از فیلتر ارتفاع‌ها).")
    ...
    # 🧩 Split سراسری ثابت (در global_multi_root_dir)
    ...
    for scen in SCENARIOS:
        ...
        # ⬇ سناریوها در زیرپوشهٔ Global_training_with_height ذخیره می‌شوند
        ...
        ds_train, ds_val = make_datasets(X_train_list, Y_train_list, X_val_list, Y_val_list)
        ...
        model = build_model(2, PAD, loss_fn)
        print(model.summary())
...
# ============================================================== #
    ...
    for h_tag in height_tags:
        ...
        # مسیر ذخیره اسکیلرها برای این ارتفاع
        height_model_dir = os.path.join(root_model_dir, h_tag)
        os.makedirs(height_model_dir, exist_ok=True)

        if is_linear:
            scaler_X_path = os.path.join(height_model_dir, "scaler_X_linear.pkl")
            scaler_Y_path = os.path.join(height_model_dir, "scaler_Y_linear.pkl")
        else:
            scaler_X_path = os.path.join(height_model_dir, "scaler_X_nonlinear.pkl")
            scaler_Y_path = os.path.join(height_model_dir, "scaler_Y_nonlinear.pkl")
        ...
        split_idx_path = os.path.join(height_model_dir, f"split_idx_seed{SEED}.npy")
        ...
        for scen in SCENARIOS:
            ...
            model_dir = os.path.join(height_model_dir, scen_name)
            os.makedirs(model_dir, exist_ok=True)
            ...
            loss_fn = make_weighted_mse_loss(PAD, ALPHA, THRESH)
            model = build_model(1, PAD, loss_fn)
            print(model.summary())
